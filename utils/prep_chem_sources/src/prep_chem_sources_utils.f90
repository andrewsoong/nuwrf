...
!        print*,'EMK: isp,nsrc, found = ',isp,nsrc,emiss_spc_name(isp,nsrc)(1:len_trim(emiss_spc_name(isp,nsrc))),src_name(nsrc)
        nvp=nvp+1
    enddo
 enddo
...
use grid_dims_out
if(use_bbem /= 1 .and. use_gfedv2 /= 1 ) use_bbem_plumerise = 0

if(merge_GFEDv2_bbem == 0) then
 if(use_bbem == 1 .and. use_gfedv2 == 1) &
     stop 'simultaneous use of 3bem and gfedv2 is not allowed when the merging is not desired'
endif

 if(use_gfedv2 == 1 .and. use_gfedv3 == 1) &
     stop 'simultaneous use of gfedv2 and gfedv3 is not allowed' ! NUWRF

 if(use_gfedv2 == 1 .and. use_gfedv4 == 1) &
     stop 'simultaneous use of gfedv2 and gfedv4 is not allowed' ! NUWRF

 if(use_gfedv3 == 1 .and. use_gfedv4 == 1) &
     stop 'simultaneous use of gfedv3 and gfedv4 is not allowed' ! NUWRF
...
subroutine write_header(iunit_out,ihour,iday,imon,iyear, &
   ng,nxp,nyp,nvert,isp,nsrc,iv)
...
!---- plumerise section
!print*,'wrth: emiss_nspecies=',emiss_nspecies;call flush(6)
if(nsrc == 2) then
if(iv > 0 .and. isp <= emiss_nspecies) then  
 call get_specie_number_of_chem_mechanism(emiss_spc_name(isp,nsrc),ident_chem_mec,ident_aer,ident_aer_mode)
  write(iunit_out,*) 'plume '
